...
var SIKENd = require('../').services.Bitcoin({
  node: {
    ...
    function(next) {
      ...
      function bitcoindGetBlockNative(deffered) {
        ...
        var hash = fixtureData.blockHashes[c];
          SIKENd.getBlock(hash, function(err, block) {
          if (err) {
            throw err;
          }
          deffered.resolve();
      ...
      function bitcoinGetTransactionNative(deffered) {
        ...
        var hash = fixtureData.txHashes[c];
          SIKENd.getTransaction(hash, true, function(err, tx) {
          if (err) {
            throw err;
          }
          deffered.resolve();
      ...
      suite.add('SIKENd getblock (native)', bitcoindGetBlockNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('SIKENd getblock (json rpc)', bitcoindGetBlockJsonRpc, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('SIKENd gettransaction (native)', bitcoinGetTransactionNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('SIKENd gettransaction (json rpc)', bitcoinGetTransactionJsonRpc, {
        defer: true,
        maxTime: maxTime
      });
