...
var SIKENRPC = require('SIKENd-rpc');
...
/**
 * Provides a friendly event driven API to SIKENd in Node.js. Manages starting and
 * stopping SIKENd as a child process for application support, as well as connecting
 * to multiple SIKENd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function SIKEN(options) {
  if (!(this instanceof SIKEN)) {
    return new SIKEN(options);
  }
...
util.inherits(SIKEN, Service);

SIKEN.dependencies = [];

SIKEN.DEFAULT_MAX_TXIDS = 1000;
SIKEN.DEFAULT_MAX_HISTORY = 50;
SIKEN.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
SIKEN.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
SIKEN.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
SIKEN.DEFAULT_SPAWN_RESTART_TIME = 5000;
SIKEN.DEFAULT_SPAWN_STOP_TIME = 10000;
SIKEN.DEFAULT_TRY_ALL_INTERVAL = 1000;
SIKEN.DEFAULT_REINDEX_INTERVAL = 10000;
SIKEN.DEFAULT_START_RETRY_INTERVAL = 5000;
SIKEN.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
SIKEN.DEFAULT_TRANSACTION_CONCURRENCY = 5;
SIKEN.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
  ...
  rpcuser: 'SIKEN',
  rpcpassword: 'SIKENpassword',
  uacomment: 'bitcore'
};

SIKEN.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || SIKEN.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || SIKEN.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || SIKEN.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || SIKEN.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || SIKEN.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || SIKEN.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || SIKEN.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || SIKEN.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || SIKEN.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || SIKEN.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

SIKEN.prototype._initCaches = function() {
...
SIKEN.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
SIKEN.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
SIKEN.prototype.getPublishEvents = function() {
  return [
    {
...
SIKEN.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'SIKENd/' + name, 'total:', this.subscriptions[name].length);
};

SIKEN.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'SIKENd/' + name, 'total:', this.subscriptions[name].length);
};


SIKEN.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
SIKEN.prototype.unsubscribeBalance = function(emitter, addresses) {
...
SIKEN.prototype.unsubscribeBalanceAll = function(emitter) {
...
SIKEN.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'SIKENd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

SIKEN.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'SIKENd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
SIKEN.prototype.unsubscribeAddressAll = function(emitter) {
...
SIKEN.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = SIKEN.DEFAULT_CONFIG_SETTINGS;
...
SIKEN.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
SIKEN.prototype._expandRelativeDatadir = function() {
...
SIKEN.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in SIKENd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in SIKENd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in SIKENd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './SIKEN.conf');
  if (spawnOptions.configdir != null) {
    configPath = path.resolve(spawnOptions.configdir, './SIKEN.conf');
  }

  log.info('Using SIKEN config file:', configPath);
...
SIKEN.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of SIKENcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of SIKENcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of SIKENcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to SIKENd from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from SIKENd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from SIKENd. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that SIKENd is undergoing a reindex. ' +
             ...
             'of SIKENcore-node services will start.');
    node._reindex = true;
  }
};

SIKEN.prototype._resetCaches = function() {
...
SIKEN.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
SIKEN.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

SIKEN.prototype._initChain = function(callback) {
  var self = this;
...
SIKEN.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 4889
  };
...
SIKEN.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/SIKEN.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/SIKEN.conf';
    }
  }
  return networkPath;
};

SIKEN.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
SIKEN.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('SIKENd/hashblock', message.toString('hex'));
    }
  }

};

SIKEN.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
SIKEN.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('SIKEN Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
SIKEN.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
SIKEN.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
SIKEN.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
SIKEN.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('SIKENd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
SIKEN.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('SIKENd/rawtransaction', message.toString('hex'));
    }
...
SIKEN.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || SIKEN.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

SIKEN.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
SIKEN.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
SIKEN.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('SIKEN Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || SIKEN.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

SIKEN.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
SIKEN.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/SIKENd.pid';
...
SIKEN.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting SIKEN process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('SIKEN process unexpectedly exited with code:', code);
        log.warn('Restarting SIKEN child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('SIKEN process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn SIKENd.'));
      }
...
SIKEN.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to SIKENd.'));
    }
 ...
 */
SIKEN.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('SIKEN configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
SIKEN.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
SIKEN.prototype.syncPercentage = function(callback) {
  var self = this;
...
SIKEN.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, SIKENcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
SIKEN.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
SIKEN.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
SIKEN.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
SIKEN.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
SIKEN.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in SIKENcore is most recent to less recent
 ...
 * @param {Function} callback
 */
SIKEN.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
SIKEN.prototype._getConfirmationsDetail = function(transaction) {
...
SIKEN.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
SIKEN.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
SIKEN.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A SIKEN transaction id
 * @param {Function} callback
 */
SIKEN.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
SIKEN.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
SIKEN.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
SIKEN.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
SIKEN.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
SIKEN.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
SIKEN.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
SIKEN.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
SIKEN.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
SIKEN.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
SIKEN.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
SIKEN.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
SIKEN.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
SIKEN.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
SIKEN.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
SIKEN.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
SIKEN.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a SIKENcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
SIKEN.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
SIKEN.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
SIKEN.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
SIKEN.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
SIKEN.prototype.getInfo = function(callback) {
...
SIKEN.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
SIKEN.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('SIKENd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('SIKENd process did not exit'));
      }
 ...
 * @param {Function} callback
 */
SIKEN.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
SIKEN.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
SIKEN.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
SIKEN.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
SIKEN.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
SIKEN.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
SIKEN.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
SIKEN.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
SIKEN.prototype.getStakingInfo = function(callback) {
    var self = this,
