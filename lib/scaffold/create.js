...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'SIKEN-lib': 'git://github.com/SIKENEKIS/SIKEN-lib.git#master',
    'SIKENcore-node': 'git://github.com/SIKENEKIS/SIKENcore-node.git'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/SIKENcore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the SIKENcore-node directory and configuration
